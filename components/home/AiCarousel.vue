<template>
	<div class="relative overflow-hidden rounded-[16px] border border-white/10 bg-[#0b0e15] p-px shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)]">
		<div class="px-4 py-10 md:px-10 md:py-12">
			<p v-if="eyebrow" class="text-center font-mono text-[12px] uppercase tracking-[0.08em] text-[#ef4d2c]">{{ eyebrow }}</p>
			...
			<div class="relative mt-8 h-[280px] w-full">
				<div class="absolute left-0 top-1/2 hidden -translate-y-1/2 gap-4 lg:flex">
					...
					<div class="grid h-5 w-5 place-items-center self-center text-[#ef4d2c]">+</div>
					<div class="h-[140px] w-[112px] overflow-hidden rounded-[4px] border border-white/10 bg-black/40 p-px">
						<div class="relative h-full w-full overflow-hidden rounded-[inherit]">
				...
				<div class="absolute left-1/2 top-[122px] hidden h-9 w-9 -translate-x-1/2 place-items-center text-[#ef4d2c] lg:grid">
					<span class="text-[32px] leading-none">›</span>
				</div>

				<div class="absolute right-0 top-0 h-[280px] w-[224px] overflow-hidden rounded-[8px] border border-[#ef4d2c]/40 bg-black/40 p-px shadow-[0px_0px_40px_-8px_rgba(239,77,44,0.5)] md:right-[calc(50%-512px+40px)]">
					<div class="relative h-full w-full overflow-hidden rounded-[inherit]">
						<img :src="activeImage" alt="AI enhanced sample" class="h-full w-full object-cover transition duration-700" />
						<span class="absolute right-2 top-2 rounded-[4px] bg-[rgba(239,77,44,0.85)] px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.05em] text-white">
							{{ enhancedLabel }}
						</span>
					</div>
				</div>
			...
			<div class="mt-8 flex items-center justify-center gap-4">
				<div class="flex items-center gap-2">
					<span v-for="(item, index) in images" :key="item.src" :class="index === activeIndex ? 'h-1.5 w-6 rounded-full bg-[#ef4d2c]' : 'h-1.5 w-1.5 rounded-full bg-white/25'"></span>
				</div>
				<button class="inline-flex items-center gap-2 rounded-full bg-[#ef4d2c] px-4 py-2 text-[14px] font-medium text-white shadow-[0px_10px_24px_rgba(239,77,44,0.22),0px_4px_3px_rgba(0,0,0,0.1)] transition hover:bg-[#ff6a45]" @click="nextSlide">
