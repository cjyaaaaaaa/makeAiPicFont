...
- `LoginModal` register flow sends email verification codes through `sendEmailCodeController` at `/user/auth/email/code` with `purpose: "register"`; the resend countdown starts only after a successful API response and shows localized success/error toasts.
